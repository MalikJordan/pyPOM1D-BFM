...
def initialize_bfm_in_pom(vertical_grid, PRESET):
    """
    ...
    if PRESET == 'bfm17':
        bfm_variables1 = set_initial_conditions_bfm17()
        bfm_variables2 = set_initial_conditions_bfm17()
...
def set_initial_conditions_bfm17():
    """
    Description: Initializes BFM state variable matrix using initial variables defined by namelist data and funciton 'read_bfm_input()'
    """
    # Local Variables
    ...
    o3c = np.zeros(pom_bfm_parameters.vertical_layers)
    o3h = np.zeros(pom_bfm_parameters.vertical_layers)

    # Pelagic nutrients (mMol / m3)
    n5s = np.zeros(pom_bfm_parameters.vertical_layers)
    o4n = np.zeros(pom_bfm_parameters.vertical_layers)
    n6r = np.ones(pom_bfm_parameters.vertical_layers)

    # Pelagic detritus (respectively mg C/m3 mMol N/m3 mMol P/m3)
    r6n = r6c * p_nRc
    r6p = r6c * p_pRc
    r6s = np.zeros(pom_bfm_parameters.vertical_layers)
    r2c = np.zeros(pom_bfm_parameters.vertical_layers)
    r3c = np.zeros(pom_bfm_parameters.vertical_layers)
    ...
    # State variables for phytoplankton model
    # Pelagic diatoms (respectively mg C/m3 mMol N/m3 mMol P/m3)
    p1c = np.zeros(pom_bfm_parameters.vertical_layers)
    p1n = p1c * p_nRc
    p1p = p1c * p_pRc
    p1s = p1c * p_sRc
    p1l = p1c * p_iRc
    ...
    # Picophytoplankton (respectively mg C/m3 mMol N/m3 mMol P/m3)
    p3c = np.zeros(pom_bfm_parameters.vertical_layers)
    p3n = p3c * p_nRc
    p3p = p3c * p_pRc
    p3l = p3c * p_iRc

    # Large phytoplankton (respectively mg C/m3 mMol N/m3 mMol P/m3)
    p4c = np.zeros(pom_bfm_parameters.vertical_layers)
    p4n = p4c * p_nRc
    p4p = p4c * p_pRc
    p4l = p4c * p_iRc
    ...
    # State variables for mesozooplankton model
    # Carnivorous mesozooplankton ( mg C/m3 )
    z3c = np.zeros(pom_bfm_parameters.vertical_layers)
    z3n = z3c * p_nRc
    z3p = z3c * p_pRc

    # Omnivorous mesozooplankton ( mg C/m3 )
    z4c = np.zeros(pom_bfm_parameters.vertical_layers)
    z4n = z4c * p_nRc
    z4p = z4c * p_pRc

    # Heterotrophic flagellates (respectively mg C/m3 mMol N/m3 mMol P/m3)
    z6c = np.zeros(pom_bfm_parameters.vertical_layers)
    z6n = z6c * p_nRc
    z6p = z6c * p_pRc
    ...
    # State variables for pelagic bacteria model B1
    # Pelagic bacteria (respectively mg C/m3 mMol N/m3 mMol P/m3)
    b1c = np.zeros(pom_bfm_parameters.vertical_layers)
    b1n = b1c * p_nRc
    b1p = b1c * p_pRc
    
    # Fill matrix with bfm variable data
    bfm17_variables = np.zeros((pom_bfm_parameters.vertical_layers,bfm_parameters.num_d3_box_states))

    bfm17_variables[:,0] = o2o[:]
    bfm17_variables[:,1] = n1p[:]
    bfm17_variables[:,2] = n3n[:]
    bfm17_variables[:,3] = n4n[:]
    bfm17_variables[:,4] = o4n[:]
    bfm17_variables[:,5] = n5s[:]
    bfm17_variables[:,6] = n6r[:]

    bfm17_variables[:,7] = b1c[:]
    bfm17_variables[:,8] = b1n[:]
    bfm17_variables[:,9] = b1p[:]
 
    bfm17_variables[:,10] = p1c[:]
    bfm17_variables[:,11] = p1n[:]
    bfm17_variables[:,12] = p1p[:]
    bfm17_variables[:,13] = p1l[:]
    bfm17_variables[:,14] = p1s[:]
 
    bfm17_variables[:,15] = p2c[:]
    bfm17_variables[:,16] = p2n[:]
    bfm17_variables[:,17] = p2p[:]
    bfm17_variables[:,18] = p2l[:]

    bfm17_variables[:,19] = p3c[:]
    bfm17_variables[:,20] = p3n[:]
    bfm17_variables[:,21] = p3p[:]
    bfm17_variables[:,22] = p3l[:]

    bfm17_variables[:,23] = p4c[:]
    bfm17_variables[:,24] = p4n[:]
    bfm17_variables[:,25] = p4p[:]
    bfm17_variables[:,26] = p4l[:]

    bfm17_variables[:,27] = z3c[:]
    bfm17_variables[:,28] = z3n[:]
    bfm17_variables[:,29] = z3p[:]

    bfm17_variables[:,30] = z4c[:]
    bfm17_variables[:,31] = z4n[:]
    bfm17_variables[:,32] = z4p[:]
 
    bfm17_variables[:,33] = z5c[:]
    bfm17_variables[:,34] = z5n[:]
    bfm17_variables[:,35] = z5p[:]
 
    bfm17_variables[:,36] = z6c[:]
    bfm17_variables[:,37] = z6n[:]
    bfm17_variables[:,38] = z6p[:]

    bfm17_variables[:,39] = r1c[:]
    bfm17_variables[:,40] = r1n[:]
    bfm17_variables[:,41] = r1p[:]
 
    bfm17_variables[:,42] = r2c[:]
    bfm17_variables[:,43] = r3c[:]

    bfm17_variables[:,44] = r6c[:]
    bfm17_variables[:,45] = r6n[:]
    bfm17_variables[:,46] = r6p[:]
    bfm17_variables[:,47] = r6s[:]

    bfm17_variables[:,48] = o3c[:]
    bfm17_variables[:,49] = o3h[:]

    return bfm17_variables
